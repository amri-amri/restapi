...
`restapi.jar` is the name of the JAR you want to run, `dbUrl` is
...
In order to run the tests in your IDE, an additional database
should be created on your MySQL Server. Use the same
SQL commands you used to create the main one. Also,
the URL, username, and password have to be provided in the file
`src/test/resources/de/uni_trier/wi2/databaseTestInfo.txt`.
##Write the URL in the first, the username in the second and
the password in the third line! **Do not use whitespace in these
lines!**## Everything beyond those first three lines will be ignored.
Additionally, you have to provide that information in
`RESTAPITest.java` and `ControllerTest.java` in the
