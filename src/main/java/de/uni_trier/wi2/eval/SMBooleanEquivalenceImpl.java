...
public class SMBooleanEquivalenceImpl extends SimilarityMeasureImpl implements SMBooleanEquivalence {
    ...
    public Similarity compute(DataObject dataObject, DataObject dataObject1, SimilarityValuator similarityValuator) {
        if (!isSimilarityFor(dataObject.getDataClass(), null) || !isSimilarityFor(dataObject1.getDataClass(), null))
            return new SimilarityImpl(this, dataObject, dataObject1);

        boolean b, b1;
        if (dataObject.isBoolean()) b = ((BooleanObject) dataObject).getNativeBoolean();
        else
            b = ((BooleanObject) ((AggregateObject) dataObject).getAttributeValue(XESorAggregateAttributeNames.VALUE)).getNativeBoolean();

        if (dataObject1.isBoolean()) b1 = ((BooleanObject) dataObject1).getNativeBoolean();
        else
            b1 = ((BooleanObject) ((AggregateObject) dataObject1).getAttributeValue(XESorAggregateAttributeNames.VALUE)).getNativeBoolean();
